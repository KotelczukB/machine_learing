...
def build_agent(model, actions):
    ...
    dqn = DQNAgent(model=model, memory=memory, policy=policy, 
                  nb_actions=actions, nb_steps_warmup=int(1.5e+6), target_model_update=1e-2)
    return dqn
...
tensorboard = keras.callbacks.TensorBoard(
    log_dir='./tensorboards/keras/DQN_5_624_2', histogram_freq=1, write_graph=True, write_images=False,
...
dqn.fit(env, nb_steps=int(3e+6), visualize=False, verbose=1)
