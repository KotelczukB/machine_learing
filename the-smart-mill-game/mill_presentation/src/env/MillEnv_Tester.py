from .modules.GamePhase import GamePhase
import numpy as np
...
class MillEnviroment(gym.Env):
    ...
    def set_gameboard(self, newObsList):
        self.phase = newObsList[len(newObsList)-1]
        self.inRow = newObsList[len(newObsList)-2]
        if self.inRow == 0:
            self.notInRow = 1
        else: 
            self.notInRow = 0
    ...
    def check_on_mill(self, newPice):
        for mill in self.mills:
            if newPice in mill:
                for spot in mill:
                    if self.gameBoard[spot] != self.inRow:
                        return False
                self.killMode = True
                print('mill!', self.killMode)
                return True
            else: 
                return False
    ...
    def step(self, action):
        ...
        return (result, self.done, info, self.killMode)
